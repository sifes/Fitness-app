...
const FrontSide: React.FC<{ right: boolean | undefined }> = ({ right }) => {
    const { upperArmsIsHovered } = useCustomSelector(state => state.bodyReducer)
    const { setIsHovered } = useActions()
    return (
        <g className={upperArmsIsHovered ? right ? 'right hovered' : 'hovered' : ''} onMouseEnter={() => setIsHovered(BODY.UPPER_ARMS)} onMouseLeave={() => setIsHovered(BODY.UPPER_ARMS)} transform="matrix(0.995057, 0, 0, 0.994516, -1310.070933, -1130.180184)">
...
const BackSide: React.FC<{ right: boolean | undefined }> = ({ right }) => {
    ...
    const { setIsHovered } = useActions()
    return (
        <g className={upperArmsIsHovered ? right ? 'right hovered' : 'hovered' : ''} onMouseEnter={() => setIsHovered(BODY.UPPER_ARMS)} onMouseLeave={() => setIsHovered(BODY.UPPER_ARMS)}>
            <title>upper arms</title>
