...
const Neck: React.FC<INeck> = ({ right, back }) => {
    ...
    selectedOptions.bodyPart === BODY.NECK ? svgStyles += 'selected ' : ''
    HoveredBody === BODY.NECK ? svgStyles += 'hovered ' : ''
    return (
        <g className={svgStyles} onMouseEnter={() => setIsHovered(BODY.NECK)} onMouseLeave={() => setIsHovered(null)} onClick={() => setOptions({ name: 'bodyPart', value: BODY.NECK })}>
            {back ?
                <BackSide />
                :
                <FrontSide />
...
const FrontSide: React.FC = () => {
    return (
        ...
        </g> // i hate fucking svg with its "transform" x4
    )
}
