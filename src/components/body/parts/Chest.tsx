...
const FrontSide: React.FC<{ right: boolean | undefined }> = ({ right }) => {
    ...
    return (
        <g className={HoveredBody === BODY.CHEST ? right ? 'right hovered' : 'hovered' : ''} onMouseEnter={() => setIsHovered(BODY.CHEST)} onMouseLeave={() => setIsHovered(null)} transform="matrix(1, 0, 0, 1, 189.333545, -1043.51735)">
            <title>chest</title>
