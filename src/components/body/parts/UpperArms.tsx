...
const FrontSide: React.FC<{ right?: boolean | undefined }> = ({ right }) => {
    ...
    return (
        <g className={HoveredBody === BODY.UPPER_ARMS ? right ? 'right hovered' : 'hovered' : ''} onMouseEnter={() => setIsHovered(BODY.UPPER_ARMS)} onMouseLeave={() => setIsHovered(null)} transform="matrix(1, 0, 0, 1, -1306, -1133)">
...
const BackSide: React.FC<{ right?: boolean | undefined }> = ({ right }) => {
    ...
    const { setIsHovered } = useActions()
    return (
        <g className={HoveredBody === BODY.UPPER_ARMS ? right ? 'right hovered' : 'hovered' : ''} onMouseEnter={() => setIsHovered(BODY.UPPER_ARMS)} onMouseLeave={() => setIsHovered(null)} >
            <title>upper arms</title>
