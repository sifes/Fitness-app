...
const FrontSide: React.FC<{ right: boolean | undefined }> = ({ right }) => {
    const { shouldersIsHovered } = useCustomSelector(state => state.bodyReducer)
    const { setIsHovered } = useActions()
    return (
        <g className={shouldersIsHovered ? right ? 'right hovered' : 'hovered' : ''} onMouseEnter={() => setIsHovered(BODY.SHOULDERS)} onMouseLeave={() => setIsHovered(BODY.SHOULDERS)} transform="matrix(0.995057, 0, 0, 0.994516, -980.314216, -1223.291025)">
...
const BackSide: React.FC<{ right: boolean | undefined }> = ({ right }) => {
    ...
    const { setIsHovered } = useActions()
    return (
        <g className={shouldersIsHovered ? right ? 'right hovered' : 'hovered' : ''} onMouseEnter={() => setIsHovered(BODY.SHOULDERS)} onMouseLeave={() => setIsHovered(BODY.SHOULDERS)}>
            <title>shoulder</title>
