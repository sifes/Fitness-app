...
const Shoulders: React.FC<IShoulders> = ({ right, back }) => {
    ...
    selectedOptions.bodyPart === BODY.SHOULDERS ? svgStyles += 'selected ' : ''
    HoveredBody === BODY.SHOULDERS ? svgStyles += 'hovered ' : ''
    return (
        <g className={svgStyles} onMouseEnter={() => setIsHovered(BODY.SHOULDERS)} onMouseLeave={() => setIsHovered(null)} onClick={() => setOptions({ name: 'bodyPart', value: BODY.SHOULDERS })}>
...
const FrontSide: React.FC = () => {
    ...
    return (
        <g transform="matrix(0.995057, 0, 0, 0.994516, -980.314216, -1223.291025)">
        ...
        </g> // i hate fucking svg with its "transform" x3
    )
}
