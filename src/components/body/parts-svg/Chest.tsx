...
const Chest: React.FC<IChest> = ({ right }) => {
    ...
    selectedOptions.bodyPart === BODY.CHEST ? svgStyles += 'selected ' : ''
    HoveredBody === BODY.CHEST ? svgStyles += 'hovered ' : ''
    return (
        <g className={svgStyles} onMouseEnter={() => setIsHovered(BODY.CHEST)} onMouseLeave={() => setIsHovered(null)} onClick={() => setOptions({ name: 'bodyPart', value: BODY.CHEST })}>
            <FrontSide />
        </g>
    )
}
const FrontSide: React.FC = () => {
    return (
        ...
        </g> // i hate fucking svg with its "transform" x9
    )
}
